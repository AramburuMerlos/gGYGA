...
for(i in seq_along(crops)) {
  ...
  tc_i <- trainControl(
    ...
    index = nndm_interp[[crops[i]]]$indx_train,
    indexOut = nndm_interp[[crops[i]]]$indx_test
  )
...
saveRDS(mod_list, "extrap_method/NNDM/NNDM_LOOCV_RF_model_list_interp.RDS")


# NNDM LOO CV RMSE and R2
...
fwrite(dt, "extrap_method/NNDM/NNDM_LOOCV_RF_interp_results.csv")
